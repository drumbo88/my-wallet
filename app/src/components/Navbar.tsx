...
import { AccountBalanceWallet, AttachMoney, Category, Dashboard } from '@material-ui/icons';
import { AddCard, Analytics } from '@mui/icons-material';
...
export default function Navbar() {
  ...
  const profileMenu = (
	...
	>
	  <MenuItem onClick={handleProfileMenuClose}>
		<NavLink style={({isActive}) => isActive ? linkActive : linkInactive} to='/profile'>
			My profile
		</NavLink>
	  </MenuItem>
	  <MenuItem onClick={handleProfileMenuClose}>
		<NavLink style={({isActive}) => isActive ? linkActive : linkInactive} to='/config'>
			Configuration
		</NavLink>
	  </MenuItem>
	  <Divider />
	  <MenuItem onClick={handleProfileMenuClose}>
		<NavLink style={({isActive}) => isActive ? linkActive : linkInactive} to='/logout'>
			Logout
		</NavLink>
	  </MenuItem>
	</Menu>
  ...
  const RightMenuIcon = ({item, showText}) => (
	  <Tooltip title={item.title}>
		<NavLink style={({isActive}) => isActive ? linkActive : linkInactive} to={item.link}>
			<IconButton size="large" aria-label={item.title} color="inherit">
				<Badge badgeContent={item.badge.content} color={item.badge.color || 'error'}>
					{item.icon}
				</Badge>
			</IconButton>
  ...
  const rightMenu = (
	<Menu
	  ...
	  id={mobileProfileMenuId} keepMounted
  ...
  return (
	<Box sx={{ flexGrow: 1 }}>
	  <AppBar position="static">
		<Toolbar>
		  ...
		  <Typography variant="h6" noWrap component="div" sx={{ display: { xs: 'none', sm: 'block' } }}>
			<NavLink style={linkInactive} to='/'>RBX-Wallets</NavLink>
		  </Typography>
	  ...
	  {rightMenu}
	  {profileMenu}
	  <Drawer anchor={drawer?.from as DrawerProps["anchor"]} open={drawer.opened} onClose={toggleDrawer(false)}>
        {list(/*drawer.from*/)}
      </Drawer>
	  {/*<MiniVariantDrawer />*/}
	</Box>
