...
const list = (req, res) => {
    currency_js_1.model.find({}).then((currencies) => {
...
const read = (req, res) => {
    currency_js_1.model.findById(req.params.id)
...
const update = (req, res) => {
    const { date, currency, amount, concept, source, destiny, detail } = req.body;
    currency_js_1.model.findByIdAndUpdate(req.params.id, {
        date, currency, amount,
        concept, source, destiny,
        detail
    })
...
const remove = (req, res) => {
    currency_js_1.model.findByIdAndDelete(req.params.id).then((doc) => {
